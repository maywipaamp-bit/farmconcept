...
(function () {
  ...
  /* ต้องตรงกับ Activity::TYPE_GENERAL ฝั่งเซิร์ฟเวอร์ — ประเภทที่บังคับกรอกแค่ชื่อ */
  var KIND_GENERAL = 'ทั่วไป';
  ...
  /* ---------- เช็กลิสต์ที่คุมปุ่มเผยแพร่และแถบความคืบหน้า ----------
     ต้องตรงกับกติกาใน ActivityRequest เสมอ ไม่งั้นจะกดเผยแพร่ได้แล้วโดนเซิร์ฟเวอร์ปฏิเสธ
     (หรือกลับกัน — ปุ่มกดไม่ได้ทั้งที่เซิร์ฟเวอร์ยอมรับ) */
  function checklist() {
    ...
    /* ประเภท "ทั่วไป" บังคับแค่ชื่อ — งานอย่างแบบสอบถามการเยี่ยมชมสวนไม่มีวันจัด รอบ
       หรือกลุ่มเป้าหมายให้กรอก ตรงกับที่ ActivityRequest ปล่อยเป็น nullable */
    if (state.kind !== KIND_GENERAL) {
      list.push(
    ...
    /* สองข้อนี้ผูกกับสวิตช์ที่ผู้ใช้เปิดเอง ไม่ใช่ประเภทกิจกรรม — กิจกรรมทั่วไปที่เปิดรับลงทะเบียน
       ก็ยังต้องเลือกแบบฟอร์ม เพราะเซิร์ฟเวอร์บังคับด้วย required_if เหมือนกัน */
  ...
  /* ---------- ตัวจับเวลาอัตโนมัติ ----------
     เดิมตัวนี้เรียก api.onSubmit() ทุก 60 วินาทีเมื่อฟอร์มมีการแก้ไขค้าง = บันทึกขึ้นเซิร์ฟเวอร์จริง
     แล้วเด้ง toast "บันทึกสำเร็จ" ทั้งที่ผู้ใช้ยังไม่ได้กดปุ่มบันทึก ซึ่งผิดสองชั้น:
       1. โหมดสร้างใหม่ จะ POST สร้างกิจกรรมขึ้นจริงระหว่างที่ยังกรอกไม่เสร็จ
          แล้ว redirect ออกจากฟอร์มไปหน้ารายการกลางคัน
       2. โหมดแก้ไข จะเขียนทับข้อมูลเดิมด้วยค่าที่กรอกค้างไว้ครึ่ง ๆ กลาง ๆ
     ตอนนี้เหลือหน้าที่เดียวคือ "เตือนว่ายังไม่ได้บันทึก" — ไม่แตะเซิร์ฟเวอร์และไม่ขึ้น toast
     การบันทึกจริงเกิดจากปุ่มบันทึกร่าง/บันทึกและเผยแพร่เท่านั้น */
  setInterval(function () {
    if (!state.dirty) return;
    markUnsaved();
  }, AUTOSAVE_MS);

  /* ป้ายบอกสถานะมุมล่าง — บอกว่ายังมีของค้างที่ยังไม่ได้บันทึก ไม่ใช่บอกว่าบันทึกให้แล้ว */
  function markUnsaved() {
    var el = $('ac-autosave');
    var text = $('ac-autosave-text');
    if (el) el.hidden = false;
    if (text) text.textContent = 'มีการแก้ไขที่ยังไม่ได้บันทึก';
  }
