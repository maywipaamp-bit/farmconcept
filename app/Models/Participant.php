...
class Participant extends Model
{
    ...
    /**
     * ชั้นยืนยันตัวตนที่คู่กับเบอร์โทร — รับได้ทั้งรหัสบุคคลและชื่อจริง
     *
     * รับสองแบบเพราะข้อมูลในระบบมาจากสองทาง คนที่ลงทะเบียนเองผ่าน QR ใช้รหัสบุคคลเป็นชื่อ
     * ส่วนคนที่เจ้าหน้าที่เพิ่มให้จากหลังบ้านมีชื่อจริงและรหัสคนละค่ากัน
     * ถ้ารับแบบเดียวจะมีคนกลุ่มหนึ่งเข้าระบบไม่ได้เลยทั้งที่กรอกถูกตามที่หน้าจอบอก
     *
     * รหัสบุคคลเทียบแบบตรงทั้งรหัส ไม่ใช่ "ขึ้นต้นด้วย" — ไม่งั้นพิมพ์ P000 แล้วชนกับ P0001
     * P0002 พร้อมกันหมด กลายเป็นเดาว่าเป็นใครไม่ได้ทั้งที่ควรจะชี้ตัวได้แน่นอน
     *
     * ส่วนชื่อเทียบแบบขึ้นต้นด้วย เพราะภาษาไทยนับตัวอักษรไม่ตรงกับที่ตาเห็น
    ...
    public function matchesNamePrefix(string $prefix): bool
    {
        ...
        if (mb_strtolower(trim((string) $this->person_code)) === $typed) {
            return true;
        }

        $first = preg_split('/\s+/', trim((string) $this->name), -1, PREG_SPLIT_NO_EMPTY)[0] ?? '';
